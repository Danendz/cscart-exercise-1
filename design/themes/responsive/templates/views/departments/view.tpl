    ...
    {strip}
        {foreach from=$splitted_departments item="sdepartments"}
            ...
            <div class="ty-column{$columns}">
                {if $department}
                {$supervisor_info = $department.supervisor_id|fn_get_user_short_info}
				{$department_url = "departments.department?department_id={$department.department_id}"|fn_url}

                {assign var="obj_id" value=$department.department_id}
                {assign 
                var="obj_id_prefix" 
                value="`$obj_prefix``$department.department_id`"}
...
{else}
    <p class="ty-no-items">{__("text_no_departments")}</p>
{/if}
